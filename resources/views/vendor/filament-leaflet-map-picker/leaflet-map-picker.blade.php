...
<script>
    const defaultLat = {{ $getDefaultLocation()['lat'] }};
    const defaultLng = {{ $getDefaultLocation()['lng'] }};
    const markerOptions = { draggable: true };

    const map = L.map('{{ $getId() }}').setView([defaultLat, defaultLng], {{ $getDefaultZoom() }});
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);

    const marker = L.marker([defaultLat, defaultLng], markerOptions).addTo(map);
    const popupHtml = `<a href="https://maps.google.com/?q=${defaultLat},${defaultLng}" target="_blank">Google Map дээр нээх</a>`;
